...
- (NSString *)emailMessageString
{
    ...
    NSString *autoPkgrLink = [NSString stringWithFormat:@"This report was generated by %@",  [@"AutoPkgr" html_link:@"https://github.com/lindegroup/autopkgr"]];

    [message appendString:[autoPkgrLink html_paragraphWithCSSClass:@"footer"]];
...
- (NSString *)toolsStatusString
{
    ...
    for (LGTool *tool in _tools) {
        if (tool.status == kLGToolUpdateAvailable) {
            if (!string) {
                string = [[@"Updates for helper tools:" html_paragraphWithCSSClass:@"status"] mutableCopy];
                [string appendString:html_openListUL];
            }
